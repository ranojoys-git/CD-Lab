```c
#include<iostream>
#include<conio.h>             // For using getch() - used in legacy systems
#include<string.h>
...
// Function to check if a character is a variable in the grammar
...
// Function to find the closure of an item
void findclosure(int z, char a)
{
    ...
    for(i=0;i<arr[z];i++)
    {
        // Loop through the characters in the right-hand side of the item
        for(j=0;j<strlen(clos[z][i].rhs);j++)
        {
            // If a dot is found followed by the character 'a'
    ...
    // Loop through the closure set again to find more items
    for(i=0;i<n;i++)
    {
        // Loop through the characters in the right-hand side of the item
        for(j=0;j<strlen(clos[noitem][i].rhs);j++)
        {
            // If a dot is followed by a non-terminal
            if(clos[noitem][i].rhs[j]=='.' && isvariable(clos[noitem][i].rhs[j+1])>0)
            {
                // Loop through the grammar rules
                for(k=0;k<novar;k++)
                {
                    // If the non-terminal matches, add the corresponding items to the closure set
                    if(clos[noitem][i].rhs[j+1]==clos[0][k].lhs)
                    {
                         // Check if the item is already in the closure set
                        for(l=0;l<n;l++)
    ...
    arr[noitem]=n;
    // Check for duplicate closure sets
    int flag=0;
    for(i=0;i<noitem;i++)
    {
        if(arr[i]==n)
...
int main()
{
    ...
    // Parse the input and build the grammar
    for(n=0;n<i-1;n++)
    {
        m=0;
        j=novar;
    ...
    // Initialize the closure set with the augmented grammar
    for(i=0;i<novar;i++)
    {
...
```
![image](https://github.com/PriyanshiNegi01/CD/assets/121029180/8097a86a-38d9-4ab3-87d0-ed5ee844fce4)
